...
namespace :action_restart do
  ...
  task before_beyond_restart: :environment do
    # 計画していた通知時間がきているユーザーをチェックして格納
    details_real_allots = DetailRealAllot.where(end_time: nil)
    details_real_allots.each do |details_real_allot|
      ...
      real_allot = RealAllot.find_by(verb_id: verb_id, created_at: 1.days.ago.all_day)
